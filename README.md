...
The conductor runs one plan per deliverable, resolved as `plans/<deliverable>.yaml`. The `/pitch <company name>` and `/earnings-update <company name>` slash commands launch a full build directly: they preset the deliverable + subject company, then collect the **whole locked questionnaire in a single interactive dialog** (`scripts/deck_spec.py`, rendered via the `AskUserQuestion` tool — clickable options plus a free-text "Other" box, the same questions with the same options every run). One call, because only the judgement items are asked: `/pitch` asks **four** — public/private, analyst notes, acquisition-target slides, and the Key Investment Highlights slide — and `/earnings-update` asks **one**, public/private. Four is the tool's own per-call maximum, so every other input is defaulted rather than asked. **Nothing is asked about attachments:** once the dialog is answered, the conductor posts a single plain-text request listing every document the run needs, split into REQUIRED and OPTIONAL with each bullet stating what the run loses without that file, and waits **once** for the analyst to drop the files into chat. Then a **single** echo reports everything that was defaulted, for override by reply — sector/industry (researched on the web), client name, presentation date, the LTM quarters (inferred from the attached filings, hence after the drop), the valuation range and risk notes (none unless you say otherwise), and the slide-mix fallbacks:

- **`earnings-update.yaml`** — decomposed quarterly earnings update, 7 stages (5 dispatched, 2 in-process) in 4 dependency waves: `wireframe` / `ltm-metrics` (parallel) → `content` / `captable` → `deck` → `deckread` / `deckcheck` (parallel). Clones the shared slide library; the `ltm-metrics` and `captable` stages write their tabs of the deal's single workbook. Fixed 5-slide layout (no slide options).
- **`pitch.yaml`** — the INFOR Slide Library pitch deck (16-slide base), 12 stages (9 dispatched, 3 in-process) scheduled into 7 dependency waves: `wireframe` / `financial-summary` / `comps` / `precedents` (parallel) → `content` / `ltm-metrics` → `captable` → `ownership` → `deck` → `financial-charts` (draws the Financial Summary charts + overview LTM-revenue pie on the deal workbook and inserts them into the deck) → `deckread` / `deckcheck` (parallel). Three deck-spec inputs adjust the slide mix: **acquisition-target slides** (1–4 slides, two targets each), **Financial Summary slides** (1 slide / 4 metrics or 2 slides / 8 metrics), and the **Key Investment Highlights slide** (include or omit). Clones the shared library; the cap-table, LTM-metrics, ownership (Canadian targets), comps, precedents and financial-summary stages each write one tab of the deal's single `pitch-<codename>.xlsx`.
- **`overview.yaml`** — stub; the overview deck is registered as a deliverable but not yet implemented.

Both plans end in the same **review pair**, running together on the finished deck and reported when the run is done: `deckread` reads every rendered slide (plus each rasterised range and chart at native resolution) and says what it *saw* — text over text, colliding shapes, faint contrast, label pileup, an unreadable pasted table; `deckcheck` reads the source filings and says which figures it could not *confirm*. Both are advisory: they are the review an analyst reads before a deck leaves the building, and neither can halt a run.
...
**Implemented:** `conductor`, `earningsupdate-content`, `pitch-content`, `captable`, `ltm-metrics`, `comps`, `precedents`, `ownership`, `financial-summary`, `deckread`, `deckcheck`.
