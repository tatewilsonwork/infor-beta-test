...
**Judgment — a dispatched sub-agent with a real tool allow-list, one `skills/<name>/SKILL.md` each.** The ten that exist: `captable`, `comps`, `precedents`, `ownership`, `financial-summary`, `ltm-metrics`, `earningsupdate-content`, `pitch-content`, `deckread`, `deckcheck`.
...
`scripts/stage_transforms.py` holds those four ports and the registry that classifies every stage. The plan YAMLs still name a transform in their `skill:` field, and that is correct — for a transform the string is its key in `TRANSFORMS`, not a directory under `skills/`. `conductor` is the eleventh `skills/` directory but is not a stage of anything: it is the meta-skill that dispatches the first list and calls the second.

The final wave of both shipped plans is the **review pair**, and it is two stages because the deck raises two unrelated questions about itself. `deckread` reads the rendered slides and reports what it *saw*; `deckcheck` reads the filings and reports which figures it could not *confirm*. Both are advisory by construction (`SlideFinding` / `CheckFinding` refuse any severity but `advisory`), both run on the finished artefact — `financial-charts`'s deck in the pitch plan, `deck`'s in the earnings update — and neither may ever gate. `deckread` exists because the `deck` transform's `vision_review_path` is an **agenda**: it asks which slides to look at and answers nothing, and for four releases no stage referenced it, so a run's own picture defect went out on a slide that file listed by name. An output nothing consumes is an output the plan only believes it has — when you write one, name the stage that reads it.
...
from conductor import (  # the conductor driver — everything mechanical about running a plan
    ...
    read_notes, suspect_currency,  # summary notes come from a FILE; argv eats every `$`
)
...
from deckread import (  # the reading pass — the READER of deck's vision_review_path
    read_deck, write_worklist,      # the mechanical half: renders, crops, the work list
    existing_renders,               # rebuild the evidence in a second process, no re-render
    SlideFinding, READ_KINDS, NOT_YOURS,   # advisory by construction, like CheckFinding
    write_report as write_deckread_report,
)
```
...
- **A transform leaves a transcript, like every other stage.** `run_transforms` tees each transform's stdout + stderr into `stages/<id>/log.txt` — the same file `write_stage_log` puts a sub-agent's in — and writes it on the failure path too, with the traceback appended. A tee, not a redirect, because both readers are real: the live run needs the converge loop's progress (a deck 40s into a repair pass and a hung one look identical otherwise) and the run directory needs it afterwards. Through v0.5.51 the eight judgment stages of a pitch run each had a log and the four transforms had none, so the only record that the repair loop shrank slide 7 to 85%, or finished on "18 blocking / 4 advisory finding(s)", was stdout in a shell — for the one stage whose entire job is making and justifying geometry decisions. `converge_deck` now also emits each surviving finding, because a converged deck deletes its own QA scratch and the count alone is unactionable.
- **Deleting a stage's SKILL.md deletes whatever that prose made mandatory — find it and re-home it, to a STAGE if it needs a reader.** Two of the four Phase F transforms ended in an analyst-facing QA section that was genuine judgment, and nothing would have failed if it had simply gone. `financial-charts`' chart check re-homed cleanly, to the declared `charts_inserted` / `pie_inserted` booleans plus rendered QA slides. The deck's "read the slides" pass did not, and the miss survived four releases: it became `vision_review_path`, a written **agenda** — every entry a question, no entry an answer — that no stage referenced and nobody read, so a picture defect shipped on a slide that file listed by name. Half a QA pass is not a QA pass. Writing the evidence is a transform's job; *reading* it is judgment, so it needs a dispatched stage (`deckread`), and a reference from that stage's `inputs:` is the only thing that makes the evidence load-bearing. Prose obligations are invisible to tests; so is an output nothing consumes.
...
- **The driver names every `Path` output at the wave boundary; the model names none of them.** `complete_wave` reads the plan's own `type: Path` labels and puts each produced path under "Open:" in the stage's checkpoint surface (`StageResult.paths`, `_paths_block`, `WaveOutcome.path_outputs`), so posting `outcome.narration()` verbatim is sufficient. A `null` is dropped — `ownership` legitimately emits `workbook_path: null` and "open None" is worse than silence. This was an instruction in the conductor SKILL.md ("name that path in the surface") and a real run's wave-5 boundary named nothing, so a 19 KB vision review the analyst was told to read was never mentioned to them. Anything the driver can derive from the plan is not an instruction to be remembered on turn 40.
- **Analyst-facing text that reaches Python must not travel through argv.** `write_run_summary`'s `notes` are the case in point: the conductor called it from a `python3 -c "…"` inside a double-quoted shell command, so `$34` / `$150` / `$MM` expanded to nothing and **every** dollar figure in one run's summary was destroyed silently — `C$34.97` printed as `C4.97`, `+US$150.0MM` as `+US50.0MM`. The plugin was not at fault, which is exactly why the CLI now takes `--notes-file` (a file, or `-` for stdin; `conductor.read_notes`) and the SKILL.md documents that as the way. Defensively, `suspect_currency` flags a currency letter sitting straight on a digit and warns on stderr *and* in `summary.md` rather than writing it quietly — banking notes are dollar-dense, so this failure mode is guaranteed another attempt.
...
- **No shipped stage gates. A run never pauses for an approval** (v0.5.49 — A2's flip to autonomous). Every stage of every shipped plan is `informational`, `deck` included; the analyst answers the intake and then reads the reviews on a finished deliverable. The machinery is intact — `Checkpoint`, `WaveOutcome.gate`, `APPROVE_LABEL` / `HALT_LABEL` and `_checkpoint_for`'s `required` branch all stay, and checkpoints still fire at wave boundaries, so a `required` gate would hold the waves *after* its own and not its wave-mates. Do not add one back to buy confidence in the deck: what the gate stood in front of is automated (`deck_repair`'s converge loop, `deck`'s written `vision_review_path` and the `deckread` stage that answers it, `deckcheck`'s falsification pass), and not one of the four asks a question. **A stage failure is not a checkpoint** — `complete_wave` reporting `ok=False` halts the run regardless of mode, and that is now the only mid-run stop.
