...
def test_deal_dir_accepts_absolute_path():
    ctx = DealContext(
        codename="Project OpenText",
        deal_dir=Path("/Users/analyst/Documents/INFOR Deals/Project OpenText"),
        deliverable_type="pitch",
    )
...
def test_empty_codename_rejected():
    with pytest.raises(ValidationError):
        DealContext(codename="", deal_dir=Path("/tmp/x"), deliverable_type="cim")
