...
1. **Earnings update POC**
   ...
   - keeps five library entries — cover, overview, earnings summary, disclaimer, contact — and deletes the rest. Since v0.5.40 each is **located by its marker shape** (`template_layout.find_slide_by_marker`), not by index: the old `_KEEP_LIBRARY_INDICES` tuple needed a manual migration every time a slide was inserted ahead of the closers, and no longer exists. Inserting a slide into the library needs no code change. The cap table replaces `Rectangle 3` on the earnings-summary slide, over the range the cap-table workbook's `infor_cap_picture_range` name resolves to (`B15:F40` as shipped — the capitalization summary plus the Financial Metrics section and the LTM/forward Valuation Metrics rows).
...
5. If `pitch`, call `assemble_pitch_deck(...)`.
   Both assemblers verify the library and workbook layouts before touching a slide or a range (shared `template_layout` map): every library slide about to be kept, cloned, deleted, or filled is located and checked by its marker shape (extending the `Rectangle 17` FS self-discovery pattern), and each Excel picture range's **defined name** (`infor_cap_picture_range`; `infor_own_insiders_picture_range`, `infor_own_institutions_picture_range`) is checked before pasting, then resolved to the actual range. A re-ordered library or a workbook that lost those names raises `TemplateLayoutError` naming what is missing — report it to the analyst; do not work around it.
...
- Overview slide: company overview bullets — the assembler sizes the bullets box to the band above the "LTM Revenue Breakdown" header (`pptx_helpers.fit_overview_textbox`) and the converge loop sets the autofit `fontScale` from the **measured** render. Do not hand-resize this box or hand-tune its scale. The cap table is pasted into the `Rectangle 3` placeholder when `captable_workbook_path` is supplied, over the range `infor_cap_picture_range` resolves to (`B15:F40` as shipped), and the `[x]$MM` footnote token is set to the cap table's output currency (`US` / `C`, read via `infor_cap_output_ccy` — `F5` as shipped). The LTM revenue pie stays a deferred placeholder; `financial-charts` later finds this slide by that very placeholder, so never remove it.
- Slide 8 (+9 when the plan carries two FS slides): financial metric **NAME** labels only, taken from the `financial_metric_labels` stage input (the `financial-summary` stage's output, not the content bundle) — four tiles per slide, in order; with two slides they are retitled `Financial Summary (1 of 2)` / `(2 of 2)` and the extra slide is cloned from the library's FS slide. The chart placeholders are left as-is here; they are filled by the downstream **`financial-charts`** stage, which runs after this one (it edits the assembled deck). Its charts read the deal workbook's `financial-summary` tab, whose LTM links point at the sibling `ltm-metrics` tab and resolve from the moment both are written.
