...
Repository cleanup plus earnings-update fixes surfaced by Test #5 (OTEX). The cleanup is behaviour-neutral; the **Fixed** items below change the earnings-update deck/workbook output. Version not bumped yet — bump every skill's `version:` frontmatter (and the plugin/marketplace/pyproject manifests) to the next release when cutting it.
